program CocheAlquiler;

uses
  Vcl.Forms,
  U_principal in 'U_principal.pas' {Form2},
  U_module in 'U_module.pas' {DataModule3: TDataModule},
  U_Accesorios in 'U_Accesorios.pas' {Form1},
  U_Caract in 'U_Caract.pas' {Form3},
  U_Clientes in 'U_Clientes.pas' {fromcli},
  U_Coche in 'U_Coche.pas' {Formcoche},
  U_CochesAlquilados in 'U_CochesAlquilados.pas' {FormCochAlq},
  U_Alquiler in 'U_Alquiler.pas' {FormAlq},
  U_CaractCoche in 'U_CaractCoche.pas' {Form4},
  U_CaractCocheCalend in 'U_CaractCocheCalend.pas' {Form5},
  U_BuscarCliente in 'U_BuscarCliente.pas' {Form6},
  U_VerContactBuscado in 'U_VerContactBuscado.pas' {Form7},
  U_BuscarPorNom in 'U_BuscarPorNom.pas' {Form8},
  U_BuscarCochePorMatri in 'U_BuscarCochePorMatri.pas' {Form9},
  U_BuscarVarios in 'U_BuscarVarios.pas' {Form10},
  Vcl.Themes,
  Vcl.Styles,
  U_Alquiler_Acces in 'U_Alquiler_Acces.pas' {Form13},
  U_Det_Alquiler in 'U_Det_Alquiler.pas' {Form14},
  U_AcercaDe in 'U_AcercaDe.pas' {Form11};

{$R *.res}

begin
  Application.Initialize;
  Application.MainFormOnTaskbar := True;
  TStyleManager.TrySetStyle('Aqua Graphite');
  Application.CreateForm(TForm2, Form2);
  Application.CreateForm(TDataModule3, DataModule3);
  Application.CreateForm(TForm1, Form1);
  Application.CreateForm(TForm3, Form3);
  Application.CreateForm(Tfromcli, fromcli);
  Application.CreateForm(TFormcoche, Formcoche);
  Application.CreateForm(TFormCochAlq, FormCochAlq);
  Application.CreateForm(TFormAlq, FormAlq);
  Application.CreateForm(TForm4, Form4);
  Application.CreateForm(TForm5, Form5);
  Application.CreateForm(TForm6, Form6);
  Application.CreateForm(TForm7, Form7);
  Application.CreateForm(TForm8, Form8);
  Application.CreateForm(TForm9, Form9);
  Application.CreateForm(TForm10, Form10);
  Application.CreateForm(TForm13, Form13);
  Application.CreateForm(TForm14, Form14);
  Application.CreateForm(TForm11, Form11);
  Application.Run;
end.
